  ...
  outResource:(NSString **)resource
{
	...
	//NSLog(@"parse 中的jidStr:%@",jidStr);
    
	if (atRange.location != NSNotFound)
	{
        //NSLog(@"位置：%i",atRange.location);
		rawUser = [jidStr substringToIndex:atRange.location];
		
		NSString *minusUser = [jidStr substringToIndex:atRange.location];
        //之前  截取用户名
        //NSString *minusUser = [jidStr substringFromIndex:atRange.location+1];
		//NSLog(@"name:%@",minusUser);
        //之前
		//NSRange slashRange = [minusUser rangeOfString:@"/"];
        
        NSRange slashRange = [jidStr rangeOfString:@"/" options:NSBackwardsSearch];
		
		if (slashRange.location != NSNotFound)
		{
            //rawDomain = [minusUser substringToIndex:slashRange.location];
			rawDomain = [jidStr substringWithRange:NSMakeRange(atRange.location+1, slashRange.location-atRange.location-1)];
            //NSLog(@"domain:%@",rawDomain);
		...
		else
		{
            ...
            //之前
			//rawDomain = minusUser;
            rawDomain=[jidStr substringFromIndex:atRange.location+1];
            rawResource=@"IOS";
		}
	}
	else
	...
	if ([XMPPJID validateUser:prepUser domain:prepDomain resource:prepResource])
	{
        ...
        if(resource) *resource = rawResource;
		//NSLog(@"if(user) prepUser %@",rawUser);
		return YES;
	}
