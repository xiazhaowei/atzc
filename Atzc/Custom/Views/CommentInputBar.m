...
{
    if (!_textField) {
        _textField = [[UITextField alloc]initWithFrame:CGRectMake(8, 5, 255, 30)];
...
{
    if (!_sendBtn) {
        _sendBtn = [UIFactory createButton:@"comment_btn.png" highlighted:@"comment_btn.png"];
        [_sendBtn setFrame:CGRectMake(_textField.right+5, _textField.top, 45, 30)];
...
#pragma mark keyboardNotification 键盘显示隐藏的通知

//- (void)keyboardWillShow:(NSNotification*)notification{
//    
//    CGRect _keyboardRect = [[[notification userInfo] objectForKey:UIKeyboardFrameEndUserInfoKey] CGRectValue];
//    //NSLog(@"高度变化：%f-%f-%f-%f",_keyboardRect.origin.y,_keyboardRect.size.height,[self getHeighOfWindow]-CGRectGetMaxY(self.frame),CGRectGetMinY(self.frame));
//    
//    //如果self在键盘之下 才做偏移
...
//                             animations:^{
//                                 self.transform = CGAffineTransformMakeTranslation(0, -_keyboardRect.size.height+[self getHeighOfWindow]-CGRectGetMaxY(self.originalFrame)-60);
//                             } completion:nil];
//        }
//        else
//        {
